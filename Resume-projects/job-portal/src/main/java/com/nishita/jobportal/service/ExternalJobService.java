...
@Service public class ExternalJobService {
 ...
 public List<ExternalJobResponse> search(String query,String location){String q=clean(query),l=clean(location);return load(q).stream().filter(j->q.isEmpty()||contains(j.title(),q)||contains(j.company(),q)||contains(j.skills(),q)).filter(j->locationMatches(j.location(),l)).limit(24).toList();}
